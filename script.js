...
// }
export default function () {
    let response = http.get('http://0.0.0.0:5555/api/schedule/98/seats-status')
    // let response = http.get('http://prod-order.irantic.com/schedule/956163')
    check(response, {
        'status is 200': (r) => r.status === 200,
    });
...
// export default function () {
//     let response = http.get('http://0.0.0.0:4444/schedule/98/seats_status?seller_id=1')
//     // let response = http.get('http://prod-order.irantic.com/schedule/956163')
//     check(response, {
//         'status is 200': (r) => r.status === 200,
//     });
...
export default function () {
    let response = http.get('https://irantic.com/')
    check(response, {
        'status is 200': (r) => r.status === 200,
    });
}